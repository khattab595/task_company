...
class _EasyStepperState extends State<EasyStepper> {
  ...
  void _afterLayout(_) {
    ...
    for (int i = 0; i < widget.steps.length; i++) {
      _scrollController!.animateTo(
        i * ((widget.stepRadius * 1.9 + (widget.internalPadding / 2)) +
                widget.lineLength),
        duration:
  ...
  List<Widget> _buildEasySteps() {
    return List.generate(widget.steps.length, (index) {
      return widget.direction == Axis.horizontal
        ...
        children: <Widget>[
          _buildStep(index),
          _buildDottedLine(index, Axis.horizontal, lineCustomColor: _buildStep(index).step.customLineColor),
        ],
      )
          : Column(
        children: <Widget>[
          _buildStep(index),
          _buildDottedLine(index, Axis.vertical, lineCustomColor: _buildStep(index).step.customLineColor),
        ],
      );
    });
  }
  ...
  Widget _buildDottedLine(int index, Axis axis, {Color? lineCustomColor}) {
      ...
      children: [
        Padding(
          ...
          child: EasyLine(
            ...
            color: lineCustomColor ?? _getLineColor(index),
